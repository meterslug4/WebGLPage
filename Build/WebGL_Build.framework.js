...
var ASM_CONSTS = {
  2871728: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 2871789: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 2871853: function() {return Module.webglContextAttributes.powerPreference;},  
 2871911: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 2871966: function($0) {performance.now = function() { return $0; };},  
 2872014: function($0) {performance.now = function() { return $0; };},  
 2872062: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _WebRTCAddICECandidate(id, candidateJson) {
          var pc = window.peerConnections[id];
          if (pc) {
              var candidate = new RTCIceCandidate(JSON.parse(UTF8ToString(candidateJson)));
  ...
  function _WebRTCCreateAnswer(id, offerJson) {
          var pc = window.peerConnections[id];
          if (pc) {
              var offer = new RTCSessionDescription(JSON.parse(UTF8ToString(offerJson)));
              pc.setRemoteDescription(offer)
                  .then(() => pc.createAnswer())
                  .then((answer) => pc.setLocalDescription(answer))
                  .then(() => {
                      if (typeof SendMessage === 'function') {
                          SendMessage('WebRTCWrapper', 'HandleAnswer', JSON.stringify(pc.localDescription));
                      }
                  })
                  .catch((error) => {
  ...
  function _WebRTCCreateOffer(id) {
          var pc = window.peerConnections[id];
          if (pc) {
              pc.createOffer()
                  ...
                  .then(() => {
                      if (typeof SendMessage === 'function') {
                          SendMessage('WebRTCWrapper', 'HandleOffer', JSON.stringify(pc.localDescription));
                      }
                  })
                  .catch((error) => {
  ...
  function _WebRTCCreatePeerConnection() {
          if (!window.peerConnections) {
              window.peerConnections = {};
          }
  
          var id = Object.keys(window.peerConnections).length;
          ...
          pc.onicecandidate = function (event) {
              if (event.candidate) {
                  ...
                  if (typeof SendMessage === 'function') {
                      SendMessage('WebRTCWrapper', 'HandleICECandidate', JSON.stringify(event.candidate));
                  }
              }
          };
          ...
          pc.onconnectionstatechange = function () {
              ...
              if (typeof SendMessage === 'function') {
                  SendMessage('WebRTCWrapper', 'HandleConnectionStateChange', pc.connectionState);
              }
          };
  
          window.peerConnections[id] = pc;
  
          console.log("WebRTC PeerConnection created with ID:", id);
          return id;
      }

  function _WebRTCSetRemoteDescription(id, answerJson) {
          var pc = window.peerConnections[id];
          if (pc) {
              var answer = new RTCSessionDescription(JSON.parse(UTF8ToString(answerJson)));
  ...
  function _WebSocketConnect(url) {
          var ws = new WebSocket(UTF8ToString(url));
          
          ws.onopen = function () {
              console.log("WebSocket connected");
              if (typeof SendMessage === 'function') {
                  SendMessage('WebSocketWrapper', 'HandleOnOpen', '');
              }
          };
          ...
          ws.onmessage = function (evt) {
              ...
              if (typeof SendMessage === 'function') {
                  SendMessage('WebSocketWrapper', 'HandleOnMessage', evt.data);
              }
          };
          ...
          ws.onclose = function () {
              ...
              if (typeof SendMessage === 'function') {
                  SendMessage('WebSocketWrapper', 'HandleOnClose', '');
              }
          };
  ...
  function _WebSocketSend(message) {
          if (window.webSocketInstance) {
              window.webSocketInstance.send(UTF8ToString(message));
          }
      }
...
var asmLibraryArg = {
  ...
  "WebRTCAddICECandidate": _WebRTCAddICECandidate,
  "WebRTCCreateAnswer": _WebRTCCreateAnswer,
  "WebRTCCreateOffer": _WebRTCCreateOffer,
  "WebRTCCreatePeerConnection": _WebRTCCreatePeerConnection,
  "WebRTCSetRemoteDescription": _WebRTCSetRemoteDescription,
